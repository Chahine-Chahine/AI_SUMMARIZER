...
function Demo() {
    ...
    const handlesubmit = async (e) => {
        e.preventDefault();
        try {
            const { data } = await getSummary({ articleUrl: article.url }).unwrap();
            if (data?.summary) {
                ...
                <div className="flex flex-col gap-1 max-h-60 overflow-y-auto">
                    {allArticles.map((item, index) => (
                        <div key={`link-${index}`} onClick={() => setArticle(item)} className="link_card">
                            ...
                            <p className="flex-1 font-satoshi text-blue-700 font-medium text-small truncate">{item.url}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="my-10 max-w-full flex justify-center items-center">
                {isFetching ? (
                    ...
                    <p className="font-inter font-bold text-black text-center">
                        We'll, that was not supposed to happen...
                        <br />
                        <span className="font-satoshi font-normal text-gray-100">{error.message}</span>
                    </p>
                ) : (article.summary && (
